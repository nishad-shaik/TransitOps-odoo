...
def enum_col(py_enum, name, **kwargs):
    """Native MySQL ENUM storing the human-readable .value, not the Python member name."""
    return Column(
...
MYSQL_TABLE_OPTS = {"mysql_engine": "InnoDB", "mysql_charset": "utf8mb4"}
# Enums
...
class Vehicle(Base):
    ...
    type = Column(String(100), nullable=False)  # drives dashboard filter
    region = Column(String(100), nullable=False)  # drives dashboard filter
...
class Trip(Base):
    ...
    active_vehicle_slot = Column(
        CHAR(36),
        Computed("CASE WHEN status IN ('Scheduled','Ongoing') THEN vehicle_id ELSE NULL END", persisted=True),
    )
    active_driver_slot = Column(
        CHAR(36),
        Computed("CASE WHEN status IN ('Scheduled','Ongoing') THEN driver_id ELSE NULL END", persisted=True),
    )
    ...
    __table_args__ = (
        ...
        UniqueConstraint("active_vehicle_slot", name="uq_one_active_trip_per_vehicle"),
        UniqueConstraint("active_driver_slot", name="uq_one_active_trip_per_driver"),
        MYSQL_TABLE_OPTS,
    )
